...
def apcluster(s, p, sparse=False, maxits=500, convits=50, dampfact=0.5, plot=False, details=False,nonoise=False):#maxits=500,convits=50, dampfact=0.5,  plot=False, details=False,nonoise=False):
    ...
    ##check inputs
    if sparse == True:
        return apcluster_sparse(s, p, maxits=maxits, convits=convits, dampfact=dampfact, plot=plot,
    ...
    if not nonoise:
        rns = np.random.get_state()
        np.random.seed(0)
        S = S + (eps * S + realmin * 100) * np.random.random(
...
def estimateNumClusters(X, eps, realmin, realmax):
