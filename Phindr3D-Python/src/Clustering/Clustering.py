...
class Clustering:
    ...
    def estimateNumClusters(self, X):
        ...
        #np.savetxt('/home/kjok/phindr_matlab_numpy/prefPY.txt', pref)
        np.savetxt('/home/kjok/phindr_matlab_numpy/CSPY_sklearn.txt', C.S)
    ...
    def getBestPreference(x, y, pl=True):
        ...
        if pl:
            ...
            idx, val = min(enumerate(x), key=lambda x: abs(x[1] - kl.knee))
            print(kl.x, kl.y, idx, ii)
            winc = clusterdisplay(kl.x, kl.y,kl.x[idx], kl.y[idx], 'Cluster Estimation kneepoint', 'Preference',
                                  '# Clusters', 'Optimal Cluster', [xCent, yCent, optText], 1)
            #kl.plot_knee()
        return yp
