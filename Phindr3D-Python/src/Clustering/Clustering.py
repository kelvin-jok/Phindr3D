...
class setcluster(object):
    def __init__(self, clusternum):
        ...
        btn_ok.clicked.connect(lambda: self.confirmed_cluster(clusterset, win))
        win.show()
        win.setWindowFlags(win.windowFlags() | Qt.CustomizeWindowHint | Qt.WindowStaysOnTopHint)
        win.exec()
    def confirmed_cluster(self, num, win):
        if num.value()>0:
            ...
            alert.setText("Number of Clusters must be a positive value")
            alert.setIcon(QMessageBox.Critical)
            alert.exec_()

class piechart(object):

    def __init__(self, plot_data, datafilt, numclusters, labels, colors): #idx from apclusterk, clusters=clusterresult, datafilt after filtering
        win = QDialog()
        win.setWindowTitle("Piechart")
        win.setLayout(QGridLayout())
        #z=np.array(datafilt[grouping], dtype='object')
        z=np.array(labels)
        clusters, count, idx=Clustering().computeClustering(datafilt, numclusters)
        print(clusters)
        print(idx)
        Utreatments=np.unique(labels)
        self.main_plot = MplCanvas(self, width=10, height=10, dpi=100, projection="2d")
        colors = plt.cm.get_cmap('gist_ncar')(range(0, 255, floor(255 / len(np.unique(labels)))))

        for i in np.unique(idx):
            ind=np.where(idx==i)
            self.main_plot.axes.plot(plot_data[0][ind], plot_data[1][ind], 'ok', alpha=0.4)
            i1= plot_data[0][ind]*np.ones(np.shape(ind)[0])
            i2 = plot_data[1][ind] * np.ones(np.shape(ind)[0])
            print(ind[0][1:])
            print(ind)
            for x in ind[0]:
                self.main_plot.axes.plot([plot_data[0][x], plot_data[0][i]], [plot_data[1][x], plot_data[1][i]], 'k-', alpha=0.2)
                #self.main_plot.axes.plot(*zip(*[(plot_data[0][x], plot_data[0][i]), (plot_data[1][x], plot_data[1][i])]))
               # self.main_plot.fig.add_artist(lines.Line2D([plot_data[0][x], plot_data[0][i]], [plot_data[1][x], plot_data[1][i]]))
        #self.main_plot.axes.invert_xaxis()
        ...
        # scalingFact = 5;
        rsize=count
        #rsize=[np.count_nonzero(idx==i) for i in clusters]
        ...
        #each cluster percent of clusters
        #start a cluster then count in index
        #np.where(idx==cluster)
        #enumerate(self.labels)
        #tot_cluster=self.labels[np.where(idx==cluster)]
        #cluster_percent=[[self.labels[np.where(idx == i)].count(trt) for trt in Utreatments] for i in clusters]
        print(np.where(idx == 0)[0].astype(int))
        print(labels[np.where(idx == 0)[0].astype(int)])
        print(np.count_nonzero(labels[np.where(idx == 0)[0].astype(int)] == 0))
        cluster_percent=[np.array([np.count_nonzero(labels[np.where(idx == i)[0].astype(int)]==trt)/np.count_nonzero(idx==i) for trt in Utreatments]) for i in clusters]
        #cluster_percent=[[self.labels[np.where(idx==i)].count(trt)/idx.count(i) for trt in Utreatments]  for i in clusters]
        ...
        for pt, cluster in zip(cluster_percent, clusters):
            ...
            for x in range(len(parts)):
                ...
                #else:
                #    self.main_plot.axes.scatter(plot_data[0][cluster], plot_data[1][cluster], marker=np.row_stack([[0, 0], np.column_stack([0, 1])]),s=4000 * rsize[size_ind], facecolor=colors[np.where(pt==1)[0]])
            size_ind=size_ind+1
            ...
            #for cluster in cluster_percent:
            #    self.main_plot.axes.pie(cluster, Utreatments, autopct='%.2f', colors=colors)
            #print(counts)
        #self.main_plot.axes.invert_xaxis()
        ...
        for i in range(len(clusters)):
            ...
            wedges, texts, autotexts = self.main_plot.axes.pie(counts, labels=labels, autopct=lambda pct: self.percent(pct, counts))
            self.main_plot.axes.legend(wedges, labels, loc='center right', bbox_to_anchor=(0.8, 0.5, 0.5, 0.5))
            self.main_plot.fig.set_facecolor('white')
        '''
...
class Clustering:
    ...
    # computeClustering.m
    def computeClustering(self, data, numberClusters, type='AP', sparse=False, maxits=500, convits=15, dampfact=0.5,
        ...
        print(C.pmed)
        if type == 'AP':
            #clusterResult = self.apclusterK(C.S, numberClusters) #percent_Dev
            ...
            print(C.pmed)
            clusterResult = self.apcluster_sklearn(data, numberClusters, sparse=sparse, maxits=maxits, convits=convits,
