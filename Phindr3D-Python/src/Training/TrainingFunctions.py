...
import numpy as np
import pandas as pd
from ..GUI.windows.featurefilegroupingwindow import *
'''
class selectWindow(object):
    def __init__(self, chk_lbl, col_lbl, win_title, grp_title, col_title, groupings):
        ...
        win.layout().addWidget(ok_button, 1, 1, 2, 2)
        ...
        if len(chk_lbl)>0:
            ...
            win.layout().addWidget(grp_vbox, 0, 0, 1, 1)
        ...
        # add columns to layout
            ...
            win.layout().addWidget(ch_checkbox, 0, 1, 1, 1)
    ...
    def selectclasses(self, mv, lbls):
        ...
        if len(np.unique(lbls))>1:
            ...
            if win.x_press:
                ...
                self.random_forest_model(X_train, y_train, X_test, lbls[y_test])
            else:
                errorWindow("Select Classes", "Must select at least two classes. Selected Classes '{}'".format(select_grps))
        else:
