...
def treatment_bounds(self, data, bounds):
    try:
        treat = data['Treatment'].str.replace(r'\\', '/', regex=True).iloc[id]
        bound = [bounds[0][np.where(data['Treatment'].unique() == treat)[0]][0], bounds[1][np.where(data['Treatment'].unique() == treat)[0]][0]]
        return(bound)
    except:
...
def merge_channels(data, rgb_img, ch_len, scroller_value, color, meta_loc, box, bounds, IntensityThreshold):
    ...
    for ind, rgb_color in zip(range(scroller_value, scroller_value + ch_len), color):
        ...
        #print(self.intensityThreshold) #list in list [[a, b, c]]
