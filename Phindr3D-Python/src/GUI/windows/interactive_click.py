...
class shift_press():
    def __init__(self, main_plot, picked_pt, cid):
        self.main_plot=main_plot
        self.picked_pt = picked_pt
        self.cid=cid
    def __call__(self, event):
        if event.key == 'shift':
            self.main_plot.axes.disable_mouse_rotation()
            self.cid=self.main_plot.fig.canvas.mpl_connect('pick_event', self.picked_pt)
            #self.main_plot.fig.canvas.mpl_disconnect(self.cid)

class shift_release():
    def __init__(self, main_plot, projection, cid):
        self.main_plot=main_plot
        self.projection=projection
        self.cid=cid
    def __call__(self, event):
        if event.key == 'shift' and self.projection == "3d":
...
class interactive_points():
    ...
    def channel_display(self, slicescrollbar, img_plot, color, x, label, cur_label, index, feature_file, file_info, ch_info, imageID):
            if feature_file:
                ...
                if len(self.labels)>1:
                    ...
                    met_loc=stacks.index[0]
                    stacks=stacks.shape[0]
                    file_info.setText("Filename: " + data['Channel_1'].str.replace(r'\\', '/', regex=True).iloc[met_loc+slicescrollbar.value()])
                else:
                ...
                slicescrollbar.setTickInterval(stacks)

                # initialize array as image size with # channels
                rgb_img = Image.open(data['Channel_1'].str.replace(r'\\', '/', regex=True).iloc[met_loc+slicescrollbar.value()]).size
                ...
                for ind, rgb_color in zip(range(slicescrollbar.value(), slicescrollbar.value() + ch_len),color):
                    ...
                    cur_img = np.array(Image.open(data['Channel_' + ch_num].iloc[met_loc+slicescrollbar.value()]))
    ...
    def __call__ (self, event): #picker is mouse scroll down trigger
        if event.mouseevent.inaxes is not None and event.mouseevent.button is MouseButton.RIGHT:
            ...
            print(self.data[0][label_ind[imin]],
                                        self.data[1][label_ind[imin]],
