...
class resultsWindow(QDialog):
    def __init__(self, color):
        ...
        prevdata.clicked.connect(lambda: self.import_file(map_type, colordropdown, threed))
        #setup Matplotlib
    ...
    def import_file(self, map_dropdown, colordropdown, threed):
        filename= QFileDialog.getOpenFileName(self, 'Open Plot Data File', '', 'Text files (*.txt)')[0]
        if filename != '':
            print(filename)
            new_data = np.loadtxt(filename)
            self.plot_data= new_data[:-6].reshape(3, int((len(new_data)-6)/3))
