...
class resultsWindow(QDialog):
    def __init__(self, color, metadata):
        ...
        controls = classification.addAction("Select Controls")
        ...
        controls.triggered.connect(lambda: TrainingFunctions().selectcontrols(np.array(self.filtered_data), np.array(self.labels)) if len(self.plot_data)>0 else errorWindow("Error Dialog","Please Select Feature File. No data is currently displayed"))
