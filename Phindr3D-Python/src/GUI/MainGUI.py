...
class MainGUI(QWidget, external_windows):
    ...
    def img_display(self, slicescrollbar, img_plot, sv, mv, color, values, img_id):
        ...
        if self.metadata.GetMetadataFilename():
            ...
            treat=""
            bound=self.bounds[:]
            if np.shape(bound[0])[0]>1:#if self.metadata.intensityNormPerTreatment: #TEMPORARY replace with setvoxelparameter intensityNormPerTreatment after completed!!!
            ...
            data['image_bounds']= np.repeat([self.bounds], data.shape[0], axis=0).tolist()
