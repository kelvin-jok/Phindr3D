    ...
    def img_display(self, slicescrollbar, img_plot, sv, mv, color, values, img_id):
        ...
        if self.metadata.GetMetadataFilename():
            ...
            if 'image_bounds' not in data.columns and 'intensity_thresholds' not in data.columns:
                ...
                data.insert(loc=int(np.where(data.columns == 'ImageID')[0]), column='intensity_thresholds', value=np.repeat([self.thresh], data.shape[0], axis=0).tolist())
                #data.insert(loc=0, column='bounds', value=np.repeat([self.bounds], data.shape[0], axis=0).tolist())
                #data.insert(loc=0, column='intensity_thresholds', value=np.repeat([self.thresh], data.shape[0], axis=0).tolist())
            else:
